...
- 引用ブロックや引用フレーズ(原文の直接引用)の近くには、必ず引用元のURLを書くこと
- 引用ブロックでは出典もブロック内に含める(最終行を `>（出典: ...）` にする)
...
- 同じ出典を同一記事内で複数回引用する場合、初出はフルタイトル、2回目以降は `[同発表記事](URL)` のような短い表記でよい
- 引用の近くに出典URLを書いた場合、そのURLは記事末尾の「参考資料」セクションから削除してよい(重複させない)
...
## 秘匿すべき情報の扱い
idやパスワードなど秘匿性の高い情報は記事に絶対に載せないこと。実際の値の代わりに、必ず `{名前}` や `< 説明 >` 形式のプレースホルダーで示すこと。

対象となる情報の例:

- ユーザーID・アカウントID・メールアドレス
- パスワード・APIキー・トークン・秘密鍵
- ARNなどのクラウドリソース識別子(アカウントIDや固有IDを含むため)
- 環境固有のドメイン・IPアドレス・リソース名
...
`articles/*.md` の編集後には `.claude/scripts/scan-secrets.mjs` が自動でスキャンされるが、検出は完全ではないため、執筆時点でプレースホルダー化しておくこと。